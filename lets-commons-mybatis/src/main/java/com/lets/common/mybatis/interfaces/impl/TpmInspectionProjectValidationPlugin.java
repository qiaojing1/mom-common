...
import com.lets.platform.model.tpm.entity.TpmInspectionProject;
...
/**
 * @author zhangweiyan
 * @ClassName TpmInspectionProjectValidationPlugin
 * @Description 巡检项目引用校验
 * @Date 2024/12/12 18:10
 **/
@Component
@Slf4j
public class TpmInspectionProjectValidationPlugin implements ValidationPluginInterface {
    @Autowired
    ...
    public void validate(List<BaseEntity> entities) {
        if (CollectionUtil.isNotEmpty(entities)) {
            if (entities.get(0) instanceof TpmInspectionProject) {
...
//                }
    ...
    private static String getQuerySql(String idStr) {
        return "select count(1) from `lets-platform-tpm`.tpm_inspection_project_route_plan_items " +
