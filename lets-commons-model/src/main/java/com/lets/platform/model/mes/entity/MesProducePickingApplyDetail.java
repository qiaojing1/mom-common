...
@Getter
@Setter
@TableName("mes_produce_picking_apply_detail")
@ApiModel(value = "MesProducePickingApplyDetail对象", description = "生产领料申请单-明细")
public class MesProducePickingApplyDetail extends BaseEntity {
    ...
    @TableField("apply_use_number")
    private BigDecimal applyUseNumber;
    @JsonFormat(shape = JsonFormat.Shape.STRING)
    @ApiModelProperty(value = "已领用数量", hidden = true)
    @TableField("already_use_number")
    private BigDecimal alreadyUseNumber;
    @ApiModelProperty(value = "领用占用数量", hidden = true)
    @TableField("occupy_use_number")
    private BigDecimal occupyUseNumber;
    @ApiModelProperty("可领料数量=申请数量-已领料数量-领用占用数量")
    @TableField(exist = false)
