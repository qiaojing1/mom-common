...
public enum SpotOrderStatus {
    ...
    private SpotOrderStatus(String code, String message) {
        this.code = code;
    ...
    public static String getName(@NotBlank(message = "code不能为空") String code) {
        SpotOrderStatus[] values = SpotOrderStatus.values();
        for (SpotOrderStatus roleType : values) {
    ...
    public static SpotOrderStatus getEnumByCode(@NotBlank(message = "code不能为空") String code) {
        SpotOrderStatus[] values = SpotOrderStatus.values();
        for (SpotOrderStatus roleType : values) {
            if (roleType.getCode().equals(code)) {
                return roleType;
