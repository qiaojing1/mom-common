...
@Getter
@Setter
@TableName("tpm_inspection_project")
@ApiModel(value = "TpmInspectionProject对象", description = "巡检项目")
public class TpmInspectionProject extends BaseEntity {
    ...
    @ApiModelProperty("巡检项分组编码")
    @TableField("code")
    @ExcelIgnore
    private String code;

    @ApiModelProperty("巡检项分组id（如果当前数据是分组则该字段为0）")
    @TableField("group_id")
    @ExcelIgnore
     ...
     */
    @ApiModelProperty("适用设备范围 5:生产单元 4:设备(该参数仅在当前数据为分组数据时有效)")
    ...
    @ApiModelProperty("巡检项目分组关联适用范围")
    @TableField(exist = false)
    @ExcelIgnore
    private List<TpmInspectionProjectGroupSuitableRange> suitableRanges;
    ...
    @TableField(exist = false)
    @ExcelIgnore
    private List<TpmInspectionProject> children;
